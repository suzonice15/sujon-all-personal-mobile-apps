...
export default function TopEarnersScreen({ navigation }) {
  ...
  const load = async () => {
    try {
      ...
      if (cached?.top_earners) {
        setData(cached.top_earners);
      }
    } catch (e) {}
    setLoading(false);
  };
  ...
  return (
    <SafeAreaView style={s.container}>
      <View style={[s.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={s.headerTitle}>শীর্ষ আয়কারী</Text>
      ...
      <View style={{ flex: 1 }}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, paddingBottom: 20 }}>
          <View style={s.banner}>
            ...
            <Text style={s.bannerTitle}>শীর্ষ আয়কারী</Text>
